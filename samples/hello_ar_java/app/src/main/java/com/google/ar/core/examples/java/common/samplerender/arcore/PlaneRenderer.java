...
public class PlaneRenderer {
    ...
    private final float[] planeAngleUvMatrix = new float[4]; // 2x2 rotation matrix applied to uv coords.
    ...
    public PlaneRenderer(SampleRender render) throws IOException {
        ...
        shader = Shader.createFromAssets(render, VERTEX_SHADER_NAME, FRAGMENT_SHADER_NAME, /*defines=*/ null)
    ...
    public void drawPlanes(SampleRender render, Collection<Plane> allPlanes, Pose cameraPose, float[] cameraProjection) {
        // 더 가까운 평면을 먼저 그려서 더 먼 평면을 가리도록 평면을 카메라로부터의 거리별로 정렬해야합니다.
        ...
        Collections.sort(sortedPlanes, (a, b) -> Float.compare(b.distance, a.distance));
